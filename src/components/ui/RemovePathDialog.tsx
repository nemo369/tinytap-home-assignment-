import { Dialog, Transition } from '@headlessui/react';
import { Fragment } from 'react';

export default function RemovePathDialog({dialogId,setDialogId,removePuzzlePiece}: {dialogId: string, setDialogId: (id: string) => void, removePuzzlePiece: (id: string) => void}) {
  return (
    <Transition show={!!dialogId} as={Fragment}>

    <Dialog open={!!dialogId} onClose={() => setDialogId('')}
    >
      <div className="fixed inset-0 bg-black/30" aria-hidden="true" />

      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Transition.Child
          as={Fragment}
          enter="ease-out duration-300"
          enterFrom="opacity-0 scale-95"
          enterTo="opacity-100 scale-100"
          leave="ease-in duration-200"
          leaveFrom="opacity-100 scale-100"
          leaveTo="opacity-0 scale-95"
        >
          <Dialog.Panel className="mx-auto max-w-md rounded bg-white px-6 py-8">
            <Dialog.Title className="text-sm  text-gray-500">Remove Piece</Dialog.Title>
            <Dialog.Description className="text-lg bold mb-10">
              Are you sure you want to delete this piece?
            </Dialog.Description>
            <div className="flex gap-x-4 py-6 justify-end" >
              <button onClick={() => setDialogId('')}
                className='bg-gray-500 text-white px-6 py-2 rounded'
              >Cancel</button>
              <button
                className='bg-red-500 text-white px-6 py-2 rounded flex gap-x-2'
                onClick={() => {
                  removePuzzlePiece(dialogId)
                  setDialogId('');
                }}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                </svg>
                <span>Delete</span>

              </button>

            </div>
          </Dialog.Panel>
        </Transition.Child>
      </div>
    </Dialog>
  </Transition>
  )
}
